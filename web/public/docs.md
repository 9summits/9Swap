...
Quotes work with zero configuration. Building a tx, simulating, `--browser` and `max` need an RPC (see 09). The first-run prompt only fires on an interactive terminal; in scripts and agents nothing ever blocks on stdin.
...
| `--nofee` | Zero the partner fee for this run. |
...
`swap` with no arguments opens the full interactive dApp locally, the same UI as https://swap.9summits.io served from your machine with your own keys. Set `NO_BROWSER_OPEN=1` to start the server without opening a browser.
